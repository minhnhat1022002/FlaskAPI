    ...
    def _create_lightgcn_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            users_embed_next, items_embed_interaction = tf.split(all_embed_interaction, [self.n_users, self.n_items],
                                                                 0)
            fusion_embed_item = [items_embed_interaction, items_embed_distance]
            fusion_embed_item = tf.stack(fusion_embed_item, 1)
            items_embed_next = tf.reduce_sum(fusion_embed_item, axis=1, keepdims=False)
