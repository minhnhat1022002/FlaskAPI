...
class LightGCN(object):
    ...
    def _init_weights(self):
        ...
        if self.pretrain_data is None:
            ...
            '''
            initilize alpha for each layer with layer 0 included
            '''
            alphainitializer = tf.keras.initializers.ones()
            for k in range(self.n_layers+1):
                all_weights['alpha_' + str(k)] = tf.Variable(alphainitializer(shape=(1,)),
                                                        name='alpha_' + str(k))
        else:
    ...
    def _create_lightgcn_embed(self):
        ...
        all_embed = [tf.multiply(ego_embed,self.weights['alpha_0'][0])]
