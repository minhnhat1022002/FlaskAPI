...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        '''
        self.u_g_embeddings_pre = [tf.nn.embedding_lookup(self.weights['user_embedding_0'], self.users)]
        self.pos_i_g_embeddings_pre = [tf.nn.embedding_lookup(self.weights['item_embedding_0'], self.users)]
        self.neg_i_g_embeddings_pre = [tf.nn.embedding_lookup(self.weights['item_embedding_0'], self.users)]
        for n in range(self.n_layers):
            self.u_g_embeddings_pre.append(tf.nn.embedding_lookup(tf.multiply(self.weights['user_embedding_0'],self.weights['user_embedding_'+str(n+1)]), self.users))
            self.pos_i_g_embeddings_pre.append(tf.nn.embedding_lookup(tf.multiply(self.weights['item_embedding_0'],self.weights['item_embedding_'+str(n+1)]), self.pos_items))
            self.neg_i_g_embeddings_pre.append(tf.nn.embedding_lookup(tf.multiply(self.weights['item_embedding_0'], self.weights['item_embedding_'+str(n+1)]), self.neg_items))
        '''
    ...
    def _init_weights(self):
        ...
        if self.pretrain_data is None:
            for n in range(self.n_layers+1):
                all_weights['user_embedding_'+str(n)] = tf.Variable(initializer([self.n_users, self.emb_dim]),
                                                            name='user_embedding_'+str(n))
                all_weights['item_embedding_'+str(n)] = tf.Variable(initializer([self.n_items, self.emb_dim]),
                                                            name='item_embedding_'+str(n))
    ...
    def _create_lightgcn_embed(self):
        ...
        users_embed = self.weights['user_embedding_0']
        items_embed = self.weights['item_embedding_0']

        ego_embed = tf.concat([users_embed, items_embed], axis=0)
        self.u_g_embeddings_pre.append(tf.nn.embedding_lookup(users_embed, self.users))
        self.pos_i_g_embeddings_pre.append(tf.nn.embedding_lookup(items_embed, self.pos_items))
        self.neg_i_g_embeddings_pre.append(tf.nn.embedding_lookup(items_embed, self.neg_items))
        ...
        for k in range(0, self.n_layers):
            users_embed = self.weights['user_embedding_'+str(k+1)]
            items_embed = self.weights['item_embedding_'+str(k+1)]
            ego_embed_k = tf.concat([users_embed, items_embed], axis=0)
            ...
            ego_embed_k = tf.multiply(ego_embed,ego_embed_k)
            u, i = tf.split(ego_embed_k, [self.n_users, self.n_items], 0)
            self.u_g_embeddings_pre.append(tf.nn.embedding_lookup(u, self.users))
            self.pos_i_g_embeddings_pre.append(tf.nn.embedding_lookup(i, self.pos_items))
            self.neg_i_g_embeddings_pre.append(tf.nn.embedding_lookup(i, self.neg_items))

            all_embed += [ego_embed_k]
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        ...
        regularizer = 0
        for n in range(len(self.u_g_embeddings_pre)):
            regularizer += tf.nn.l2_loss(self.u_g_embeddings_pre[n]) + tf.nn.l2_loss(
                self.pos_i_g_embeddings_pre[n]) + tf.nn.l2_loss(self.neg_i_g_embeddings_pre[n])
