...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        """
        self.leaky_relu_layer = tf.keras.layers.LeakyReLU(negative_slope=0.5) # leaky relu use in message propagation
        ...
        *********************************************************
        ...
        for n in range(self.n_layers):
            ...
            self.u_g_embeddings_pre.append(tf.nn.embedding_lookup(self.weights['user_embedding_'+str(n+1)], self.users))
            self.pos_i_g_embeddings_pre.append(tf.nn.embedding_lookup(self.weights['item_embedding_'+str(n+1)], self.pos_items))
            self.neg_i_g_embeddings_pre.append(tf.nn.embedding_lookup(self.weights['item_embedding_'+str(n+1)], self.neg_items))
        ...
        for k in range(0, self.n_layers):
            ...
            for K in range(0, self.n_layers):
                ...
                for fold in range(self.n_fold):
                    temp_embed_interaction.append(tf.sparse_tensor_dense_matmul(A_fold_hat[fold], ego_embed_k))
                all_embed_interaction = tf.concat(temp_embed_interaction, axis=0)

                ego_embed_k = all_embed_interaction

            all_embed += [self.leaky_relu_layer(ego_embed_k)]
