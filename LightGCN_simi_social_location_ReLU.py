...
class GCCF(object):
    ...
    def _create_gccf_embed(self):
        ...
        SI_fold_hat = self._split_I_hat(self.similar_item_adj)
        Social_fold_hat = self._split_U_hat(self.social_adj)
        ...
        for k in range(0, self.n_layers):
            ...
            for fold in range(self.n_fold):
                temp_embed_similar_users.append(tf.sparse_tensor_dense_matmul(SI_fold_hat[fold], items_embed))
            ...
            users_embed_interaction, item_embed_interaction = tf.split(all_embed_interaction,
                                                                       [self.n_users, self.n_items],
                                                                       0)
            fusion_embed_users = [users_embed_interaction, users_embed_social]
            fusion_embed_users = tf.stack(fusion_embed_users, 1)
            users_embed_next = tf.reduce_sum(fusion_embed_users, axis=1, keepdims=False)

            fusion_embed_items = [item_embed_interaction, items_embed_distance, users_embed_similar]
